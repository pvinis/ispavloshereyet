					...
					<li
						...
						className={cn(
							...
							index === currentLocationIndex &&
								"text-xl font-bold text-white hover:motion-preset-blur-right",
							// Upcoming locations
							index < currentLocationIndex && "text-emerald-400 hover:text-blue-400",
							// Past locations
							index > currentLocationIndex && "text-sm text-zinc-800 hover:text-zinc-400",
						)}
					>
						{index === currentLocationIndex ? <span className="-ml-4">→ {place}</span> : place}
						{index < currentLocationIndex && (
